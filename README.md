...
实时资金与板块领导力终端。盘中资金来自东方财富公开行情接口，基础快照与历史记忆来自同花顺扶摇 Financial API。

## 实时交易链路

- 首页：实时流入/流出 Bar，每 30 秒更新。
- 板块页：`/sector/{BK代码}`，展示分钟资金、5/30 分钟变化、内部红盘率和个股角色分层。
- 个股页：`/stock/{股票代码}`，展示实时价格、成交额、量比、换手、主力净额和基础盘口。
- Memory Bank：记录板块历史上榜、本机盘中状态、回流与失败观察规则。
